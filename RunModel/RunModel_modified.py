...
from sam.datasets._image_features_reader import ImageFeaturesH5Reader
import h5py
...
class RunModel:
    ...
    def textVQA(self, question, img_path):
        ...
        ##### get ocr tokens
        tokens = self._get_ocr_tokens(image_path)
        ###################
        ...
        with torch.no_grad():
            for batch in self.dataloaders["test"]:
                loss,score,_,predctions=forward_model(task_cfg, device, self.model, None, "pred", batch, True)
    # 해당 대화를 분석한다. predictions가 unanswerable이 아닌 것 중에서, 가장 높은 score를 가진 것을 선택한다....???
