...
def load_imdb(question, image_height, image_width,tokens):
    ...
    entry = edict(
        {
            ...
            "google_ocr_tokens_filtered": tokens
        }
    )
    entries.append(entry)
    return entries
...
class TextVQADataset(Dataset):
    ...
    def __init__(
        self, split, tokenizer, question, image_height, image_width, tokens, padding_index=0, processing_threads=32, task_cfg=None,
    ):
        super().__init__()
        self.split = split
        ...
        if not os.path.exists(cache_path) or self.debug:
            ...
            # load imdbs
            self.entries = load_imdb(self.question, self.image_height, self.image_width, self.tokens)
