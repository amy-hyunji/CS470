...
if __name__ == "__main__":
    demo = MMFDemo()
    detection_model = demo.detection_model
    url = "https://m.media-amazon.com/images/I/71Qec8xXstL._AC_UL640_FMwebp_QL65_.jpg"
    im, im_scale = demo._image_transform(url)
    img_tensor, im_scales = [im], [im_scale]
    current_img_list = to_image_list(img_tensor, size_divisible=32)
    current_img_list = current_img_list.to("cuda")
    torch_out = demo.detection_model(current_img_list)
    torch.onnx.export(
    ...
    ort_inputs = {ort_session.get_inputs()[0].name: to_numpy(current_img_list)}
